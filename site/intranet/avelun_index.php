...
?>  
 ...
 <div style="width: 250px; height: auto; background-color: none; margin: 0 auto; top; text-align: center;">
  <h3 style="margin-bottom: 50px; margin-top: 100px;">Marcas de Avelun</h3>
      <form method="post" action="#">
                    <label>Selecciona la Marca de Avelun:</label>
                    <?php

                    $consulta_mysql="SELECT * FROM marcas WHERE tienda_asociada='Avelun'";
                    $resultado_consulta_mysql= $conexion->query($consulta_mysql);
  
                    echo "<select class='form-control' name='marca' style='height:40px; width:150px; margin:0 auto;'>";
                    ?>
...
?>    
                    <br>
                    <input type="submit" name="cargar_articulos_avelun" value="Cargar" class="btn btn-dark">
    </form>
  </div>
...
if(isset($_POST['cargar_articulos_avelun'])){
                  $marca = $_POST['marca'];
